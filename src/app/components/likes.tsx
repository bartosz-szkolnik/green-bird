...
export default function Likes({
  ...
  const handleClick = async () => {
    ...
    if (tweet.userHasLikedTweet) {
      addOptimisticTweet({ ...tweet, likes: tweet.likes - 1, userHasLikedTweet: !tweet.userHasLikedTweet });
    ...
    <button onClick={handleClick} className="group flex items-center">
      <svg
        ...
        className={`group-hover:fill-red-600 group-hover:stroke-red-600 ${
          tweet.userHasLikedTweet ? 'fill-red-600 stroke-red-600' : 'fill-none stroke-gray-500'
        }`}
      >
      ...
      <span
        className={`ml-2 text-sm group-hover:text-red-600 ${
          tweet.userHasLikedTweet ? 'text-red-600' : 'text-gray-500'
        }`}
      >
        {tweet.likes}
      </span>
